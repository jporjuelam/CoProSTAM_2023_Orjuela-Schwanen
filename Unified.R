...
FixedActivities <- data.frame(Place_type=c("home","work","home"),
                              Lat=c(-75.612951,-75.580403,-75.612951),
                              Lon=c(6.182718,6.191988,6.182718),
                              Start_trip_time=c("9:00","17:00",NA), #time to leave this place, NA for last trip of the day
                              End_trip_time=c(NA,"12:30","19:00"),#time to arrive to this place, NA for first trip of the day
                              DayWeek = c(1,1,1)) 
...
FlexibleActivities <- data.frame(key=c("amenity","shop","custom"), #in order of priority
                                 value=c("hospital","convenience","visit"), #in order of priority
                                 Lat=c(NA,NA,-75.596677),
                                 Lon=c(NA,NA,6.187353),
                                 Open_time=c("9:00","6:00","18:00"),
                                 Close_time=c("17:00","20:00","20:00"),
                                 MinActivityDuration=c(40/60,30/60,1.5),#in hours
                                 Status =c(1,1,1)) #0 indicates that has already been done 

Results <- vector(mode='list',length=7)
d <- 1
for (d in 1:7){#Open day loop
FixedActivitiesDay <- FixedActivities[FixedActivities$DayWeek==d]
FixedActivitiesDayResults <- FixedActivitiesDay

l <- 1
g <- l
for (l in 1:dim(FixedActivitiesDay)[1]-1){#Open trip loop 
FlexibleActivitiesR <- FlexibleActivities[FlexibleActivities$Status==1]#Exclude flexible activities that have been done 

######Step 0 - Initialize #######
#Step 0.1
#Available modes
Modes <- data.frame("Mode" = c("bicycle","foot","motorcar","motorcycle"), 
                      "Cost" = c(0,0,0.25,0.15), #Cost as proportion of income and independent of distance (needs checking!)
                      "PM25"=c(2,1.9,2.5,2.5)) #From de Nazelle, Bode and Orjuela (2017)
...
for (m in 1 :4){
  ...
  dodgr_fastest_path <- street_network_dodgr_sc_bicycle[dodgr_fastest_ids[[1]][[1]], ] #distances in metres and times in seconds
  Shrt_T <- (sum(dodgr_fastest_path$time_weighted)/60)/60 #from seconds to minutes to hours
  ...
  if(Total_T>=Shrt_T){ #Opens space-time prism check loop
      ...
      #plot(Serv1, col="green")
      #plot(Serv2,col=alpha("blue",0.4),add=T)
      #plot(Serv3,col=alpha("yellow",0.4),add=T)
      #plot(Serv4,col=alpha("red",0.4),add=T)
      
      #Step 1.6
      ISAbig <- gIntersection(Serv1,Serv2)
      ISAsm <- gIntersection(Serv3,Serv4)
      ISA <- gIntersection(ISAbig,ISAsm)
      proj4string(ISA) <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
      ISA <- st_as_sf(ISA)
      ...
      for (i in 1:length(OPCost_id[[1]])[1]){
        ...
        OPtravel_time <- (sum(OPCost_atributes$time_weighted)/60)/60
        ...
        PDtravel_time <- (sum(PDCost_atributes$time_weighted)/60)/60
      ...
      TravelTimes$lat <- as.data.frame(st_coordinates(FOSc_test))$X
      TravelTimes$long<- as.data.frame(st_coordinates(FOSc_test))$Y
      #Step 3.5 & 3.6
      Access <- sum(TravelTimes$WAreaDur)
...
bestOptionFlex <- data.frame(Place_type=FlexibleActivitiesR$value[f],
                    Lat=as.numeric(as.character(maxTravelTimes$lat[which(maxTravelTimes$wtCost_inv==max(maxTravelTimes$wtCost_inv))])),
                    Lon=as.numeric(as.character(maxTravelTimes$long[which(maxTravelTimes$wtCost_inv==max(maxTravelTimes$wtCost_inv))])),
                    Start_trip_time=as.character(strftime(maxTravelTimes$leave[which(maxTravelTimes$wtCost_inv==max(maxTravelTimes$wtCost_inv))],format="%H:%M")),
                    End_trip_time=as.character(strftime(maxTravelTimes$arrive[which(maxTravelTimes$wtCost_inv==max(maxTravelTimes$wtCost_inv))],format="%H:%M")),
                    DayWeek=d)

FlexibleActivities$Status[f] <- 0 #Delete from flex activities (FlexibleActivitiesR)
...
} #closes if that checks for no activities 
